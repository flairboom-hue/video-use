...
`tests/` covers the helper logic that fails silently — caption offsets, cut-safety thresholds, the transcript format contract, overlay anchoring. Run `pytest` from the repo root after changing a helper; it takes under a second.
...
- **`references/youtube-retention.md`** — read when the brief is a YouTube video that has to hold the viewer, not just a clean cut. Hook structure, interrupt cadence, and the optional external tools (screen capture, Shorts repurposing, NLE handoff) with where each plugs in.
